...
export function AgendaBuilder({
  ...
  const templateItems = useMemo(
    () => Object.fromEntries(templates.map((tpl) => [tpl.id, tpl.name])),
    [templates],
  );
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        {templates.length > 0 && (
          ...
          >
            <SelectTrigger className="w-[170px] h-8 text-xs">
              ...
              <SelectValue placeholder={t("agendaBuilder.useTemplate")} />
            </SelectTrigger>
            <SelectContent>
        ...
        >
          ...
          {isGeneratingAgenda
            ? t("agendaBuilder.generating")
            : t("agendaBuilder.aiGenerate")}
        </Button>
      </div>
      ...
      {showAiInput && (
        <div className="rounded-lg border border-primary/20 bg-primary/5 p-3 space-y-2">
          <p className="text-xs text-muted-foreground font-medium">
            {t("agendaBuilder.aiPromptDescription")}
          </p>
          <div className="flex gap-2">
            <Input
              placeholder={t("agendaBuilder.aiPromptPlaceholder")}
              value={aiPrompt}
            ...
            >
              ...
              <div className="flex flex-col items-center gap-0.5 pt-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                <button
                  ...
                  aria-label={t("agendaBuilder.moveUp")}
                >
                ...
                <button
                  ...
                  className="text-muted-foreground hover:text-foreground disabled:opacity-20 p-0.5"
                  aria-label={t("agendaBuilder.moveDown")}
                >
              ...
              <button
                ...
                aria-label={t("agendaBuilder.removeItem")}
              >
      ...
      <div className="space-y-2">
        <div className="flex gap-2">
          <Input
            placeholder={t("agendaBuilder.addPlaceholder")}
            value={newTitle}
          ...
          >
            ...
            {isAddingItem
              ? t("agendaBuilder.addingItem")
              : t("agendaBuilder.add")}
          </Button>
        </div>
        {newTitle.trim() && (
          <Textarea
            placeholder={t("agendaBuilder.descriptionPlaceholder")}
