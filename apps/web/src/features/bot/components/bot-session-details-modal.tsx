...
interface BotSessionDetailsModalProps {
  ...
  isLoading?: boolean;
  /** Error from fetch (e.g. network/auth) - shown when session is null and not loading */
  error?: Error | null;
}
...
export function BotSessionDetailsModal({
  ...
  const hasSession = session != null && !isLoading;
  const noSession = session == null && !isLoading;
